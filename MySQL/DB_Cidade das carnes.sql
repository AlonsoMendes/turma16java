...
select * from tb_produtos where nome like "%Pi%"; -- mostra todos produtos com pi

select * from tb_produtos where preco in (20,30,18); -- mostra os precçs dos produtos de 20,30,18

select * from tb_produtos where preco between 20 and 40; -- mostra os preços dos produtos de 20 a 40

select  count(*) from tb_produtos; -- mostra a quantidade de produtos

select avg(preco) as media from tb_produtos; -- mostra a media de preço dos produtos

select sum(preco) from tb_produtos; -- soma o preços de todos os produtos

-- left join trata a tabela da esquerda como importante
-- right join trava a tabela da direita como importante

select  nome, preco, qtProduto, tb_categoria.descricao, tb_categoria.ativo from tb_produtos inner join tb_categoria on 
tb_categoria.id = tb_produtos.categoria_id; -- faz a junção das duas tabelas 
